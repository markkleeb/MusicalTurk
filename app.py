...
def hits():
	...
	if request.method == "POST" and song_upload_form.validate():
		...
		# get existing images
		songs = models.Song.objects(tag='hits').order_by('-timestamp')
	...
	if request.method == "POST" and song_upload_form.validate():
		...
		if uploaded_file and allowed_file(uploaded_file.filename):
			...
			if k and k.size > 0:
				...
				submitted_loop.title = request.form.get('title')
				submitted_loop.postedby = request.form.get('postedby')
				submitted_loop.tag = request.form.get('tag')
				submitted_loop.filename = filename # same filename of s3 bucket file
				submitted_loop.save()
...
def songs():
	...
	if request.method == "POST" and song_upload_form.validate():
		...
		return render_template("songs.html", **templateData)
